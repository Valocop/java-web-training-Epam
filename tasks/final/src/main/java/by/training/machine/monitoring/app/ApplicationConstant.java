package by.training.machine.monitoring.app;

public class ApplicationConstant {
    public static final String DEFAULT_ROLE = "DEFAULT";
    public static final String ADMIN_ROLE = "ADMIN";
    public static final String MANUFACTURER_ROLE = "MANUFACTURER";
    public static final SecurityService SECURITY_SERVICE = SecurityService.getInstance();
    public static final String VIEW_ALL_USERS_CMD = "viewUserList";
    public static final String SHOW_ADD_MACHINE_CMD = "showAddMachine";
    public static final String SHOW_LIST_MACHINES_CMD = "showListCars";
    public static final String SHOW_LIST_USER_MACHINES_CMD = "showUserListCars";
    public static final String ADD_CHARACTERISTIC_CMD = "addCharacteristic";
    public static final String DELETE_MACHINE_CMD = "deleteMachine";
    public static final String SAVE_MACHINE_CMD = "saveMachine";
    public static final String SHOW_MACHINE_STATUS_CMD = "showMachineStatus";
    public static final String DELETE_USER_CMD = "deleteUser";
    public static final String DELETE_USER_MACHINE_CMD = "deleteUserMachine";
    public static final String ADD_MODEL_CMD = "addModel";
    public static final String COMMAND_NAME = "commandName";
    public static final String TOAST = "toast";
}
